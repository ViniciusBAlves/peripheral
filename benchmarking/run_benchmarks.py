...
ATTEMPT_FIELDS = [
    ...
    "status", "reconnect_count", "kex_group", "kex_nist_level",
...
SUMMARY_FIELDS = [
    "case_id", "kex_group", "kex_nist_level", "kex_public_key_bytes",
...
def run_job(
    ...
    cpu_cycles = number(final, "client_cpu_cycles")
    cycle_hz = number(final, "client_cycle_hz")
    cpu_ms = cpu_cycles / cycle_hz * 1000.0 if cpu_cycles is not None and cycle_hz else None
    ...
    return {
        ...
        **case_metadata(case),
        ...
        "client_cpu_ms": f"{cpu_ms:.3f}" if cpu_ms is not None else "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pi-host", default="thiago@10.12.194.1")
    parser.add_argument(
        "--pi-workdir",
        default="",
    ...
    parser.add_argument("--ble-addr", default="")
...
def main() -> int:
    ...
    if args.dry_run:
        summaries = [
            {
                "case_id": case["case_id"], **case_metadata(case),
    ...
    universal_dir = WORK / "generated" / run_id
    generate_universal_header(
        client_dir,
        [
        ...
        universal_dir / "benchmark_credentials.h",
    )
    ...
    if not args.skip_build:
        ...
        build_firmware(
            ...
            generated_dir=universal_dir, log=run_dir / "build.log",
    ...
    if not args.skip_flash:
        print(f"[firmware] Flashing nRF52840; log={run_dir / 'flash.log'}", flush=True)
        flash_firmware(
    ...
    interrupted = False
    try:
        with serial.Serial(args.serial_device, args.serial_baud, timeout=0.25) as serial_port:
            print(
