#!/usr/bin/env python3
"""Benchmark key-exchange operations directly on the nRF52840 DK."""
...
ATTEMPT_FIELDS = [
    ...
    "client_heap_capacity_bytes", "client_heap_peak_usage_percent",
    ...
    *DWT_FIELDS,
    ...
    "dwt_counts_are_modulo", "error_code", "message",
]
...
SUMMARY_FIELDS = [
    ...
    "firmware_static_ram_used_bytes", "firmware_ram_capacity_bytes",
    "firmware_static_ram_usage_percent",
    *[f"mean_{field}" for field in DWT_FIELDS],
    "dwt_cycle_counter_supported", "dwt_event_counters_supported",
...
def attempt_row(attempt: int, kem: Kem, values: dict[str, str]) -> dict[str, object]:
    ...
    secret_mismatch = status == "success" and values.get("secrets_match", "0") != "1"
    ...
    return {
        ...
        "kem_encapsulation_ms": microseconds_as_ms(values, "kem_encapsulation_us"),
        "kem_decapsulation_ms": microseconds_as_ms(values, "kem_decapsulation_us"),
        ...
        "client_heap_peak_usage_percent": usage_percent(
            values, "client_heap_peak_bytes", "client_heap_capacity_bytes"
        ),
...
def summarize(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for group, items in sorted(groups.items()):
        ...
        heap_pct = numbers("client_heap_peak_usage_percent")
        if not heap_pct:
            heap_pct = [
                float(value) for value in (
                    usage_percent(
                        item, "client_heap_peak_bytes", "client_heap_capacity_bytes"
                    )
                    for item in success
                )
                if value != ""
            ]
        firmware_ram = numbers("firmware_static_ram_used_bytes")
        firmware_pct = numbers("firmware_static_ram_usage_percent")
        dwt_values = {field: numbers(field) for field in DWT_FIELDS}
        summaries.append({
            **{
            ...
            "cancelled_count": sum(row["status"] == "cancelled" for row in items),
            ...
            "firmware_static_ram_usage_percent": (
                f"{max(firmware_pct):.2f}" if firmware_pct else ""
            ),
            **{
                f"mean_{field}": (
            ...
            "dwt_cycle_counter_supported": (
                success[0].get("dwt_cycle_counter_supported", "") if success else ""
            ),
            "dwt_event_counters_supported": (
                success[0].get("dwt_event_counters_supported", "") if success else ""
            ),
            "dwt_counts_are_modulo": (
                success[0].get("dwt_counts_are_modulo", "") if success else ""
            ),
        })
    return summaries
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--serial-device", default=str(config.get("serial-device", "/dev/ttyACM0"))
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    build_dir = (
        WORK / "firmware-build" /
        f"kem-operations-nrf52840-{slug(args.mlkem_backend)}"
    )
    ...
    if not args.skip_build:
        ...
        build_firmware(
            ...
            pqm4_dir=pqm4_dir,
            extra_cmake_args=[
                "-DBENCH_KEM_OPERATIONS=ON",
                "-DEXTRA_CONF_FILE=kem_bench.conf",
            ],
        )
    ...
    with serial.Serial(
        ...
        for sequence, case in enumerate(cases, 1):
            ...
            for attempt in range(1, args.iterations + 1):
                ...
                serial_port.flush()
                timeout = min(args.attempt_timeout_sec, MAX_ALGORITHM_TIMEOUT_SEC)
                try:
