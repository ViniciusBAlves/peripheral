...
static void set_wifi_enabled(bool enabled)
{
    if (enabled) {
        ...
        (void)command_status("sudo nmcli radio wifi on >/dev/null 2>&1 || "
                             "sudo ip link set wlan0 up >/dev/null 2>&1 || true");
    } else {
        printf("[*] Disabling Raspberry Pi Wi-Fi during BLE benchmark.\n");
        (void)command_status("sudo nmcli radio wifi off >/dev/null 2>&1 || "
                             "sudo ip link set wlan0 down >/dev/null 2>&1 || true");
    }
}
...
static void recycle_adapter(bool full_reset)
{
    if (full_reset) {
        ...
        (void)command_status("sudo btmgmt power off >/dev/null 2>&1 || true");
        sleep(1);
        (void)command_status("sudo btmgmt bredr off >/dev/null 2>&1 || true");
        (void)command_status("sudo btmgmt bondable off >/dev/null 2>&1 || true");
        (void)command_status("sudo btmgmt sc off >/dev/null 2>&1 || true");
        (void)command_status("sudo btmgmt privacy off >/dev/null 2>&1 || true");
        (void)command_status("sudo btmgmt power on >/dev/null 2>&1 || true");
    } else {
        (void)command_status("bluetoothctl power on >/dev/null 2>&1 || true");
    }
    (void)command_status("sudo btmgmt bredr off >/dev/null 2>&1 || true");
    (void)command_status("sudo btmgmt bondable off >/dev/null 2>&1 || true");
    (void)command_status("sudo btmgmt sc off >/dev/null 2>&1 || true");
                                      ...
                                      bool reset_adapter)
{
    ...
    snprintf(command, sizeof(command),
             "bluetoothctl disconnect %s >/dev/null 2>&1 || true", address);
    (void)command_status(command);

    if (forget_cache) {
        printf("[*] Removing cached BlueZ record for %s\n", address);
        snprintf(command, sizeof(command),
                 "bluetoothctl remove %s >/dev/null 2>&1 || true", address);
        (void)command_status(command);
    }
